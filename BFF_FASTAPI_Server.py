...
callback_text_key = CustomJS(code="""
    cur_key = this.value
""")
callback_text_val = CustomJS(code="""
    cur_val = this.value
""")
...
async def modify_file(in_file: InputTemplateModel):
    ...
    # g_loc_file_run.fileContent = g_loc_file_run.fileContent +  "WTF____WTF " + str(g_loc_file_run.N)
...
async def post_content(in_file: InputTemplateModel):
    ...
    # ncontent = PreText(text=file_content, width=500, height=100)  # <pre> file_content</pre>
...
async def read_root(request: Request, inline: bool = True):
    ...
    # script3, div3 = components(row(text_input_key,text_input_val, button_modify))
